...
### Sorting
The best way to sort data in **R** is using the `order` function. It sorts value,
but returns their order, not the sorted values. It may seem strange, but is actually
very powerful.
...
If you want to see the actual values, the command would be
```{r}
...
Classify the flows by season: winter = NDJF, spring = MAM,
summer = JJA, fall = SO
