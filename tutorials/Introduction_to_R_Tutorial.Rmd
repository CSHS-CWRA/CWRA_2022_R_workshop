...
### Sorting
The best way to sort data in **R** is using the `order` function. It sorts values,
but returns their _order_, not the sorted values. It may seem strange, but is actually
very powerful.
...
system.time(for (i in 1:length(d1)) {d2[i] <- d1[i]^0.5})
...
and the seconc column as the y values, we can use the `plot` function defaults.
...
you need the `coefficients` function. If you want R², you need
to use the `summary` function as shown above.
...
within your notebook, or you can store it in its own .R file.
